"""Long-prompt regime ablation: M2.7 SLO-PID vs M1 static_ratio under chunked prefill.

Sweep dirs: results/long_chunk_seed{0,1,2}/ (3 configs × 1 qps × 60s × 3 seeds).
Configs: c1_baseline, c2_tdm_m1_chunk2048, c2_tdm_m31_2048 (chunk=2048 in all M-variants).
Question: does PID controller earn ≥ 2pp over static_ratio when prompts are long-dominated?
"""
...
RUNS = {
    "c1_baseline": [(s, ROOT / f"long_chunk_seed{s}", "c1_baseline")
                    for s in (0, 1, 2)],
    "M1+chunk2048": [(s, ROOT / f"long_chunk_seed{s}", "c2_tdm_m1_chunk2048")
                     for s in (0, 1, 2)],
    "M2.7+chunk2048": [(s, ROOT / f"long_chunk_seed{s}", "c2_tdm_m31_2048")
                       for s in (0, 1, 2)],
}
...
def main():
    ...
    tiers = [
        ...
        ("ttft1000/tpot200", 1000, 200),
        ("ttft1500/tpot150", 1500, 150),
        ("ttft1500/tpot200", 1500, 200),
    ]
    print(f"\n=== LONG-PROMPT regime (3 seeds, qps={QPS}) ===")
    ...
    # Verdict on PID effect
    print(f"\n=== PID verdict (focus tiers, M2.7+chunk - M1+chunk) ===")
    focus = [("ttft500/tpot150", 500, 150),
             ("ttft500/tpot200", 500, 200),
             ("ttft1000/tpot100", 1000, 100),
             ("ttft1000/tpot150", 1000, 150),
             ("ttft1500/tpot200", 1500, 200)]
    for label_t, tt, tp in focus:
        m1c_xs = [g[(tt, tp)] for g in grids["M1+chunk2048"]]
        m27c_xs = [g[(tt, tp)] for g in grids["M2.7+chunk2048"]]
        d = [m27c_xs[i] - m1c_xs[i] for i in range(3)]
        mu, sd = mean_std(d)
        verdict = "PID +effect" if mu >= 2.0 else ("PID nil" if abs(mu) < 1.0 else ("PID weak+" if mu > 0 else "PID weak-"))
        print(f"  {label_t:<18s}: Δ = {mu:+5.2f} ± {sd:>4.2f} pp  →  {verdict}")
